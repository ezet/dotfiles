---
name: html-artifacts
description: Produce a self-contained HTML artifact instead of markdown when content benefits from spatial layout, color, real diagrams, interactivity, or a round-trip editor. Use whenever the user asks for a document-shaped deliverable (doc, plan, spec, report, review, deck, mockup, post-mortem, one-off tool) or asks you to explain, compare, or explore a non-trivial topic — even without saying "HTML" or "artifact". Stay in markdown for short conversational replies, code-only output, and terminal-style command answers.
---
