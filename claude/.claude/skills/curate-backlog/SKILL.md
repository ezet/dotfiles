...
`start-backlog` carries a comment naming its agent; that comment is how you
tell abandoned from busy.
...
`hoopit-board` (in `~/.local/bin`) is the board's mechanical half; `--help`
lists it. `scan` prints every open item with the signals that nominate it, then
...
`hoopit-board triage <repo> <n> --priority P2 --size M`. The rubrics are in the
`create-gh-issue` skill; use those rather than inventing a second scale.
